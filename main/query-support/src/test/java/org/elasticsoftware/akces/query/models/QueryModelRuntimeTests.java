...
public class QueryModelRuntimeTests {
    ...
    @Test
    @Order(5)
    public void testRegisteredSchemas() throws IOException {
        while (!walletController.isRunning() ||
                !accountController.isRunning() ||
                !orderProcessManagerController.isRunning() ||
                !akcesClientController.isRunning() ||
        ...
        var walletCreatedSchema = schemaStorage.getSchema("domainevents.WalletCreated", 1);
        assertTrue(walletCreatedSchema.isPresent());
        assertEquals(1, walletCreatedSchema.get().version());

        var accountCreatedSchema = schemaStorage.getSchema("domainevents.AccountCreated", 1);
        assertTrue(accountCreatedSchema.isPresent());
        assertEquals(1, accountCreatedSchema.get().version());
    }
