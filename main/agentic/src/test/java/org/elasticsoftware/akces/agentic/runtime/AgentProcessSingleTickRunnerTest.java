...
class AgentProcessSingleTickRunnerTest {
    ...
    void tickShouldCallTickAndCollectEvents() {
        ...
        // The processed-events set is lazily created on first call
        when(blackboard.get(AgentProcessResultTranslator.PROCESSED_EVENTS_KEY)).thenReturn(null);
        ...
        // Events are tracked via a set stored on the blackboard, not hidden
        verify(blackboard).set(eq(AgentProcessResultTranslator.PROCESSED_EVENTS_KEY), any());
