...
class RocksDBAggregateStateRepositoryFactoryTest {

    @Test
    void testCreate() throws Exception {
        ProtocolRecordSerde serde = new ProtocolRecordSerde();
        String baseDir = "/tmp/test-rocksdb";
        
        RocksDBAggregateStateRepositoryFactory factory = 
            new RocksDBAggregateStateRepositoryFactory(serde, baseDir);
        
        AggregateRuntime aggregateRuntime = mock(AggregateRuntime.class);
        when(aggregateRuntime.getName()).thenReturn("TestAggregate");
        
        AggregateStateRepository repository = factory.create(aggregateRuntime, 0);
        
        assertNotNull(repository);
        assertTrue(repository instanceof RocksDBAggregateStateRepository);
        
        repository.close();
    }

    @Test
    void testCreateWithDifferentPartitions() throws Exception {
        ProtocolRecordSerde serde = new ProtocolRecordSerde();
        String baseDir = "/tmp/test-rocksdb";
